...
class K2CompilerFacade(environment: KotlinCoreEnvironment) : KotlinCompilerFacade(environment) {
    ...
    override fun analyze(
        ...
        return listOf(
            FirAnalysisResult(commonAnalysis, commonKtFiles, reporter),
            FirAnalysisResult(platformAnalysis, platformKtFiles, reporter)
        )
    }
    ...
    private fun frontend(
        ...
        val analysisResults = analyze(platformFiles, commonFiles)

        FirDiagnosticsCompilerResultsReporter.throwFirstErrorAsException(
            analysisResults.first().reporter,
        ...
        val fir2IrResult = FirResult(
            analysisResults.map { it.moduleCompilerAnalyzedOutput }
